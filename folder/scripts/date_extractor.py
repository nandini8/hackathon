...
def _extractFromMeta(parsedHTML):
    ...
    for meta in parsedHTML.findAll("meta"):
        ...
        #<meta property="article:published_time" content="Wed, 14 Feb 2018 07:43:00 GMT">
        if 'article:published_time' == metaProperty:
            metaDate = meta['content'].strip()
            print("5")
            print(meta['property'], meta['content'])
            if(meta['content'] == '{{pubDate}}'):
                print(datetime.strptime(meta['content'], "%m/%d/%Y"))
            break
        ...
        #<meta name="publish-date" content="2018-02-14T17:41:44+05:30">
        if 'publish-date' == metaName:
            metaDate = meta['content'].strip()
            print("17")
            break
...
if __name__ == '__main__':
    ...
    d = extractArticlePublishedDate(url)
    print(d)
